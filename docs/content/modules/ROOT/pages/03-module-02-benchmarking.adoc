...
----
guidellm benchmark \
  --target "http://maas.apps.ocp.cloud.rhai-tmm.dev/nemotron-cascade/nemotron-cascade-2-30b/v1" \
  --backend-kwargs '{"api_key": "'$TOKEN'"}' \
  --profile concurrent \
  --rate 10 \
...
----
guidellm benchmark \
  --target "http://maas.apps.ocp.cloud.rhai-tmm.dev/llama4-scout/llama-4-scout-17b/v1" \
  --backend-kwargs '{"api_key": "'$TOKEN'"}' \
  --profile concurrent \
  --rate 10 \
  --model meta-llama/Llama-4-Scout-17B-16E-W4A16 \
  --data "prompt_tokens=64,output_tokens=128" \
  --max-seconds 30
----
...
----
guidellm benchmark \
  --target "http://maas.apps.ocp.cloud.rhai-tmm.dev/llama32-3b/llama-3.2-3b-instruct/v1" \
  --backend-kwargs '{"api_key": "'$TOKEN'"}' \
  --profile concurrent \
  --rate 10 \
